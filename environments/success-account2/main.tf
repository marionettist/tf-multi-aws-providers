...
resource "aws_vpc_peering_connection" "account2_gateway" {
    peer_owner_id   = "${data.terraform_remote_state.account1.aws_account_id}"
    peer_vpc_id     = "${data.terraform_remote_state.account1.vpc_id}"
    vpc_id          = "${data.terraform_remote_state.account2.vpc_id}"
...
module "account2_route" {
  source = "../../modules/peering_route"

  aws_region                  = "${var.aws_region}"
  aws_profile                 = "${var.account2_aws_profile}"
  route_table_id              = "${data.terraform_remote_state.account2.route_table_id}"
  destination_cidr_block      = "${data.terraform_remote_state.account1.vpc_cidr}"
  vpc_peering_connection_id   = "${aws_vpc_peering_connection.account2_gateway.id}"
}
